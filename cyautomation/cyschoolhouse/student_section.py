...
def enrollment_sync(source_section, destination_section, enrollment_start_date,
    ...
    for x in [source_section, destination_section]:
        section_types = program_df['Program__c_Name'].tolist()
        if x not in section_types:
            raise ValueError(f'{x} is not a valid section type. '
                             f'Try one of: {section_types}')

    stu_sec_df = cysh.get_object_df('Student_Section__c',
    ...
    stu_sec_df = stu_sec_df.merge(section_df, how='left', on='Section__c')

    # get students enrolled in source section
    src_df = stu_sec_df.loc[
        stu_sec_df['Program__c_Name'] == source_section
    ].copy()

    # get students enrolled in destination section
    dest_df = stu_sec_df.loc[
        stu_sec_df['Program__c_Name'] == destination_section
    ].copy()
    ...
    src_df['key'] = src_df[key_cols].astype(str).sum(axis=1)
    dest_df['key'] = dest_df[key_cols].astype(str).sum(axis=1)

    if ACM_to_TL:
        src_df = src_df.drop_duplicates('key')

    to_enroll_df = src_df.loc[
        ~src_df['key'].isin(dest_df['key'])
    ].copy()

    # get set of destination sections
    dest_section_df = section_df.loc[
        section_df['Program__c_Name'] == destination_section
    ].copy()

    # merge in section ID to enroll in
    if ACM_to_TL:
        dest_cols = ['School__c', 'Section__c']
        merge_on = 'School__c'
    else:
        dest_cols = ['Intervention_Primary_Staff__c', 'Section__c']
        merge_on = 'Intervention_Primary_Staff__c'
    ...
    to_enroll_df = to_enroll_df.loc[
        ~to_enroll_df['Section__c_to_Enroll'].isnull()
    ]

    print(f"Enrolling {len(to_enroll_df)} students from {source_section} to "
          f"{destination_section} sections.")
