...
from .config import get_sch_ref_df, set_logger, TEMP_PATH, TEMPLATES_PATH
...
def fill_one_acm_wb(acm_df, acm_name, wb):
    # Write header
...
def merge_and_save_one_school_pdf(school_informal_name):
    ...
    merger.write(f"Z:/{school_informal_name} Team Documents/"
                 f"SY19 Weekly Service Trackers - {school_informal_name}.pdf")
    merger.close()
...
def update_service_trackers():
    ...
    for school in student_section_df['School_Reference_Id__c'].unique():
        # Start Excel - must reboot for each loop or Excel will crash
        app = xw.App()
        # Open blank template
        ...
        logger.info(f"Writing Service Tracker: {school}")
        ...
        for acm_name in df_school['Staff__c_Name'].unique():
            ...
            try:
                fill_one_acm_wb(acm_df, acm_name, wb)
